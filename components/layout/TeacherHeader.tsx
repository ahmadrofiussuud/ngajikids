...
import { Sparkles, Calendar, Users, LayoutDashboard, User, LogOut } from "lucide-react";

export default function TeacherHeader() {
  const pathname = usePathname();

  const navItems = [
    { name: "Kelas Saya", href: "/guru", icon: <LayoutDashboard size={16} /> },
    { name: "Jadwal & Booking", href: "/guru/schedule", icon: <Calendar size={16} /> },
    { name: "Profil Saya", href: "/guru/profile", icon: <User size={16} /> },
  ];

  const isActive = (href: string) => {
    if (href === "/guru") {
      return pathname === "/guru";
    }
    ...
    <header className="sticky top-0 z-40 bg-white border-b-3 border-neutral-border py-4 px-4 sm:px-6 shadow-sm font-nunito">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center justify-between w-full md:w-auto">
          <Link href="/guru" className="flex items-center gap-2 active:scale-95 transition-all">
            ...
            <span className="bg-emerald-100 text-emerald-700 text-[9px] font-black px-2.5 py-0.5 rounded-full border border-emerald-250">
              PENGAJAR
            </span>
          </Link>
        </div>
        ...
        <div className="flex items-center gap-3 justify-end w-full md:w-auto border-t md:border-t-0 pt-3 md:pt-0 border-gray-100">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-emerald-100 text-emerald-700 flex items-center justify-center font-black text-xs select-none">
              🎓
            </div>
